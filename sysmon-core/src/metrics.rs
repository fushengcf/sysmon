...
mod tests {
    ...
    fn test_metrics_collector_new() {
        ...
        assert!(collector.prev_instant.elapsed().as_secs() < 5);
    }
    ...
    fn test_metrics_collector_collect() {
        ...
        assert!(metrics.net_rx_kbps >= 0.0);
        assert!(metrics.net_tx_kbps >= 0.0);
    }

    #[test]
    fn test_net_only_collector_new() {
        let collector = NetOnlyCollector::new();
        assert!(collector.prev_instant.elapsed().as_secs() < 5);
    }

    #[test]
    fn test_net_only_collector_collect_net() {
        let mut collector = NetOnlyCollector::new();
        let (rx, tx) = collector.collect_net();
    ...
    #[test]
    fn test_metrics_cpu_per_core_count() {
        let mut collector = MetricsCollector::new();
        let metrics = collector.collect();

        let sys = System::new_all();
        assert_eq!(metrics.cpu_per_core.len(), sys.cpus().len());
    }
    ...
    fn test_metrics_serialization() {
        ...
        assert!(json_str.contains("memory_usage_percent"));
        assert!(json_str.contains("net_rx_kbps"));
        assert!(json_str.contains("net_tx_kbps"));
    }
    ...
    fn test_metrics_deserialization() {
        ...
        assert_eq!(deserialized.memory_usage_percent, metrics.memory_usage_percent);
        assert_eq!(deserialized.net_rx_kbps, metrics.net_rx_kbps);
        assert_eq!(deserialized.net_tx_kbps, metrics.net_tx_kbps);
    }
}
