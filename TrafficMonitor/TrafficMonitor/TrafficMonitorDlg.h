...
protected:
    ...
    // ── WebSocket 服务器 ── 对应 sysmon-ws/src/server.rs ──────────────────────
    CWebSocketServer m_ws_server;           //WebSocket 服务器实例（端口 9001）
    void ApplyWsServerState(bool enable);
    bool IsWsLightModeEnabled() const;
    void UpdateWsMenuState(CMenu* pMenu);
    void BroadcastMetrics();                //序列化当前指标并广播给所有已连接客户端
public:
...
public:
    ...
    afx_msg void OnEnableWsServer();
    afx_msg void OnDisableWsServer();
