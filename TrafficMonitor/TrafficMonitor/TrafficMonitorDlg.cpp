...
BOOL CTrafficMonitorDlg::OnInitDialog()
{
    ...
    // 启动 WebSocket 服务器（对应 sysmon-ws 的 run_server）
    if (!m_ws_server.Start(9001))
    {
        // 启动失败不影响主程序运行，仅记录日志
        TRACE(_T("[WebSocketServer] Failed to start on port 9001\n"));
    }
    else
    {
        TRACE(_T("[WebSocketServer] Listening on ws://0.0.0.0:9001\n"));
    }
...
{
    // 如果没有客户端连接，跳过序列化（节省 CPU，与 sysmon-ws 的 receiver_count == 0 优化一致）
