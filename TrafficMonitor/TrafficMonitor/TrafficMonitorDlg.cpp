...
void CTrafficMonitorDlg::ApplyWsServerState(bool enable)
{
    constexpr unsigned short kWsServerPort = 9001;
    if (enable)
    {
        if (!m_ws_server.IsRunning() && !m_ws_server.Start(kWsServerPort))
            TRACE(_T("[WebSocketServer] Failed to start on port %u\n"), kWsServerPort);
    }
    else if (m_ws_server.IsRunning())
    {
        m_ws_server.Stop();
    }
}

bool CTrafficMonitorDlg::IsWsLightModeEnabled() const
{
    return !theApp.m_cfg_data.m_ws_server_enabled;
}

void CTrafficMonitorDlg::UpdateWsMenuState(CMenu* pMenu)
{
    if (pMenu == nullptr)
        return;

    CString ws_menu_text;
    ws_menu_text.Format(_T("是否启动ws(%s)"),
        theApp.m_cfg_data.m_ws_server_enabled ? _T("是") : _T("否"));
    int ws_popup_index = -1;
    for (int i = 0; i < pMenu->GetMenuItemCount(); ++i)
    {
        CMenu* sub_menu = pMenu->GetSubMenu(i);
        if (sub_menu == nullptr || sub_menu->GetMenuItemCount() < 2)
            continue;
        if (sub_menu->GetMenuItemID(0) == ID_WS_SERVER_ENABLE &&
            sub_menu->GetMenuItemID(1) == ID_WS_SERVER_DISABLE)
        {
            ws_popup_index = i;
            break;
        }
    ...
    if (ws_popup_index >= 0)
    {
        CMenu* ws_sub_menu = pMenu->GetSubMenu(ws_popup_index);
        const UINT_PTR ws_sub_menu_handle = reinterpret_cast<UINT_PTR>(ws_sub_menu->GetSafeHmenu());
        pMenu->RemoveMenu(ws_popup_index, MF_BYPOSITION);
        pMenu->InsertMenu(ws_popup_index, MF_BYPOSITION | MF_POPUP | MF_STRING, ws_sub_menu_handle, ws_menu_text);
    }

    CString client_count_menu_text;
    client_count_menu_text.Format(_T("连接设备数：%d"),
        theApp.m_cfg_data.m_ws_server_enabled ? m_ws_server.GetClientCount() : 0);
    int client_count_index = -1;
    for (int i = 0; i < pMenu->GetMenuItemCount(); ++i)
    {
        if (pMenu->GetMenuItemID(i) == ID_WS_CLIENT_COUNT)
        {
            client_count_index = i;
            break;
        }
    }
    if (client_count_index >= 0)
    {
        pMenu->RemoveMenu(client_count_index, MF_BYPOSITION);
        pMenu->InsertMenu(client_count_index, MF_BYPOSITION | MF_STRING,
            ID_WS_CLIENT_COUNT, client_count_menu_text);
    }
    pMenu->EnableMenuItem(ID_WS_CLIENT_COUNT,
        MF_BYCOMMAND | (theApp.m_cfg_data.m_ws_server_enabled ? MF_GRAYED : MF_DISABLED | MF_GRAYED));
}
...
BEGIN_MESSAGE_MAP(CTrafficMonitorDlg, CDialog)
    ...
    ON_COMMAND(ID_WS_SERVER_ENABLE, &CTrafficMonitorDlg::OnEnableWsServer)
    ON_COMMAND(ID_WS_SERVER_DISABLE, &CTrafficMonitorDlg::OnDisableWsServer)
...
BOOL CTrafficMonitorDlg::OnInitDialog()
{
    ...
    ApplyWsServerState(theApp.m_cfg_data.m_ws_server_enabled);
...
void CTrafficMonitorDlg::DoMonitorAcquisition()
{
    ...
    lite_version = true;
#endif
    const bool ws_light_mode = IsWsLightModeEnabled();
    ...
    if (!ws_light_mode)
    {
        ...
        // 获取 GPU 利用率时优先使用 PDH 计数器。
        // 这一路通常比硬件监控库更稳定，也更接近任务管理器的占用率表现。
...
#ifndef WITHOUT_TEMPERATURE
    //获取温度
    if (!ws_light_mode && IsTemperatureNeeded() && theApp.m_pMonitor != nullptr)
    {
        CSingleLock sync(&theApp.m_minitor_lib_critical, TRUE);
    ...
    // 将最新指标广播给所有 WebSocket 客户端（对应 sysmon-ws 的 broadcast::Sender::send）
    BroadcastMetrics();
}
...
void CTrafficMonitorDlg::OnDestroy()
{
    ...
    // 停止 WebSocket 服务器（对应 sysmon-ws 的 stop_tx.send(())）
    m_ws_server.Stop();
...
{
    UpdateWsMenuState(theApp.m_taskbar_menu.GetSubMenu(0));
    UpdateWsMenuState(theApp.m_taskbar_menu_plugin.GetSubMenu(0));
    //设置“选择连接”子菜单项中各单选项的选择状态
...
void CTrafficMonitorDlg::OnEnableWsServer()
{
    if (theApp.m_cfg_data.m_ws_server_enabled)
        return;

    theApp.m_cfg_data.m_ws_server_enabled = true;
    ApplyWsServerState(true);
    UpdateWsMenuState(theApp.m_main_menu.GetSubMenu(0));
    UpdateWsMenuState(theApp.m_main_menu_plugin.GetSubMenu(0));
    theApp.SaveConfig();
}

void CTrafficMonitorDlg::OnDisableWsServer()
{
    if (!theApp.m_cfg_data.m_ws_server_enabled)
        return;

    theApp.m_cfg_data.m_ws_server_enabled = false;
    ApplyWsServerState(false);
    UpdateWsMenuState(theApp.m_main_menu.GetSubMenu(0));
    UpdateWsMenuState(theApp.m_main_menu_plugin.GetSubMenu(0));
    theApp.SaveConfig();
}
...
// ─── WebSocket 指标广播 ───────────────────────────────────────────────────────
//
// 对应 sysmon-ws/src/server.rs 的 metrics_broadcast_task：
//   · 每次 DoMonitorAcquisition() 完成后调用
//   · 将 CTrafficMonitorApp 中的实时指标序列化为 JSON，推送给所有已连接客户端
//   · JSON 格式与 sysmon-ws 的 SystemMetrics 结构对齐，便于 Android 客户端复用解析逻辑

void CTrafficMonitorDlg::BroadcastMetrics()
{
    if (!theApp.m_cfg_data.m_ws_server_enabled || !m_ws_server.IsRunning())
        return;
    if (m_ws_server.GetClientCount() <= 0)
        return;
    ...
    // 网速：TrafficMonitor 以字节/秒存储，转换为 KB/s（与 sysmon-ws 的 net_rx_kbps / net_tx_kbps 对齐）
    double net_rx_kbps = static_cast<double>(theApp.m_in_speed)  / 1024.0;
    double net_tx_kbps = static_cast<double>(theApp.m_out_speed) / 1024.0;

    // 内存：TrafficMonitor 以 KB 为单位存储
    int memory_total_mb = theApp.m_total_memory / 1024;
    int memory_used_mb  = static_cast<int>(
        static_cast<long long>(theApp.m_total_memory) * theApp.m_memory_usage / 100 / 1024);

    // 构建 JSON（手动构造，不依赖任何 JSON 库）
    // 格式与 sysmon-ws 的 SystemMetrics 结构体保持一致
    char json[1024];
    sprintf_s(json, sizeof(json),
        ...
        "}",
        timestamp,
        static_cast<double>(max(0, theApp.m_cpu_usage)),
        static_cast<double>(max(0, theApp.m_memory_usage)),
        memory_used_mb,
        memory_total_mb,
        net_rx_kbps,
        net_tx_kbps,
        max(0, theApp.m_gpu_usage),
        static_cast<double>(theApp.m_cpu_temperature < 0 ? 0 : theApp.m_cpu_temperature),
        static_cast<double>(theApp.m_gpu_temperature < 0 ? 0 : theApp.m_gpu_temperature)
    );

    m_ws_server.Broadcast(std::string(json));
}
